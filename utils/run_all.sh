...
nohup ros2 run qcar_science_night_pkg v2v_receiver_node > /tmp/v2vrx.log 2>&1 &
sleep 3

say "3/6  bird's-eye lane detection"
nohup python3 lane_bev.py > /tmp/lanebev.log 2>&1 &
sleep 3
...
nohup ros2 run qcar_science_night_pkg path_mpc --ros-args \
  ...
  -p max_reference_offset:=0.32 \
  -p enable_reference_offsets:=true \
  -p overtake_max_curvature:=1.0 -p overtake_mean_curvature:=0.60 \
  -p max_lane_offset_m:=0.12 -p min_lane_offset_m:=0.04 \
  -p lane_confidence_floor:=0.25 -p lane_centering_curve_limit:=1.20 \
  -p enable_v2v:=true \
  > /tmp/t2.log 2>&1 &
...
if [ "${1:-}" = "--arm" ]; then
  ...
  nohup ros2 run qcar_science_night_pkg lidar_overtake --ros-args \
    ...
    -p front_box_max_m:=1.80 -p emergency_box_max_m:=0.70 \
    -p side_box_max_m:=0.90 -p lane_width_m:=0.43 \
    -p emergency_half_width_m:=0.12 \
    -p front_narrow_half_width_m:=0.16 -p min_front_narrow_points:=2 \
    -p front_stop_straight_m:=1.40 -p emergency_stop_straight_m:=0.70 \
    -p front_stop_curve_m:=0.90 -p emergency_stop_curve_m:=0.65 \
    -p overtake_start_min_distance_m:=1.00 \
    -p hard_stop_front_distance_m:=0.40 \
    -p overtake_offset_m:=0.30 \
    > /tmp/t3.log 2>&1 &
  sleep 6
