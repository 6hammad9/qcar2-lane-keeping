...
public:
    ...
    : Node("qcar2")
    {
        ...
        param_desc.description = "Motor command watchdog timeout in seconds.";
        param_desc.additional_constraints = "Must be positive. Stale commands force speed and steering to zero.";
        this->declare_parameter("motor_command_timeout_sec", motor_command_timeout_sec, param_desc);

        param_desc.description = "Maximum accepted commanded linear speed in m/s.";
        param_desc.additional_constraints = "Commands are clipped symmetrically to this magnitude.";
        this->declare_parameter("max_command_speed", max_command_speed, param_desc);

        param_desc.description = "Maximum accepted commanded steering angle in radians.";
        param_desc.additional_constraints = "Commands are clipped symmetrically to this magnitude.";
        this->declare_parameter("max_command_steering", max_command_steering, param_desc);
        ...
        motor_command_timeout_sec = this->get_parameter("motor_command_timeout_sec").as_double();
        max_command_speed = this->get_parameter("max_command_speed").as_double();
        max_command_steering = this->get_parameter("max_command_steering").as_double();
        if (motor_command_timeout_sec <= 0.0)
        {
            RCLCPP_WARN(this->get_logger(),
                "Invalid motor_command_timeout_sec; using 0.30 seconds");
            motor_command_timeout_sec = 0.30;
        }
        if (max_command_speed <= 0.0)
        {
            RCLCPP_WARN(this->get_logger(),
                "Invalid max_command_speed; using 0.60 m/s");
            max_command_speed = 0.60;
        }
        if (max_command_steering <= 0.0)
        {
            RCLCPP_WARN(this->get_logger(),
                "Invalid max_command_steering; using 0.58 rad");
            max_command_steering = 0.58;
        }
...
private:
    ...
    void speed_controller()
    {
        ...
        double dt = clamp_value(delta_time.seconds(), 0.0, 0.1);
        double measured_speed = 0;

        const auto steady_now = std::chrono::steady_clock::now();
        const bool command_fresh = motor_command_received &&
            std::chrono::duration<double>(
                steady_now - last_motor_command_time).count() <= motor_command_timeout_sec;
        ...
        if (!command_fresh)
        {
            ...
            motor_speed_cmd = 0.0;
            if (!watchdog_stop_active)
            {
                RCLCPP_WARN(
                    this->get_logger(),
                    "Motor command watchdog stopped the QCar2 (timeout %.3f s).",
                    motor_command_timeout_sec);
                watchdog_stop_active = true;
            }
        }
        else
        {
            watchdog_stop_active = false;
        }

        // method used for constructing a PD speed controller for QCar2
        //Convert desired linear speed to desired motor speed
        
        if (desired_speed != 0 && battery_voltage > 1.0)
        {
            ...
            speed_error_integral = clamp_value(speed_error_integral, -0.30, 0.30);
    ...
    void motor_command_callback(const qcar2_interfaces::msg::MotorCommands &motor_commands)
    {
        ...
        if (motor_commands_size == 0 ||
            motor_commands_size > motor_channel_map.size() ||
            motor_commands.values.size() != motor_commands_size)
        {
            RCLCPP_WARN(
                this->get_logger(),
                "In %s - MotorCommands names/values must have equal size between 1 and %ld; received %ld names and %ld values...ignoring",
                __FUNCTION__, motor_channel_map.size(), motor_commands_size,
                motor_commands.values.size());
            return;
        }

        // An omitted field is deliberately zeroed instead of retaining an
        // older speed or steering command.
        ...
        for (unsigned int i = 0; i < motor_commands_size; i++)
        {
            ...
            if (!std::isfinite(motor_commands.values[i]))
            {
                RCLCPP_WARN(this->get_logger(), "In %s - non-finite motor command...ignoring", __FUNCTION__);
                valid_message = false;
                break;
            }

            if (motor_commands.motor_names[i] == "steering_angle")
                next_steering = motor_commands.values[i];
            else if (motor_commands.motor_names[i] == "motor_throttle")
                next_speed = motor_commands.values[i];
        }

        if (valid_message)
        {
            std::lock_guard<std::mutex> command_lock(command_mutex_);
            desired_steering = clamp_value(
                next_steering, -max_command_steering, max_command_steering);
            desired_speed = clamp_value(
                next_speed, -max_command_speed, max_command_speed);
            last_motor_command_time = std::chrono::steady_clock::now();
    ...
    rcl_interfaces::msg::SetParametersResult set_parameters_callback(const std::vector<rclcpp::Parameter> & parameters)
    {
        ...
        for (const auto & parameter : parameters)
        {
            ...
            else if (parameter.get_name().compare("motor_command_timeout_sec") == 0 ||
                     parameter.get_name().compare("max_command_speed") == 0 ||
                     parameter.get_name().compare("max_command_steering") == 0)
            {
                if (node_running)
                {
                    result.successful = false;
                    result.reason = "Cannot change motor safety parameters while node is running.";
                }
                else if (parameter.as_double() <= 0.0)
                {
                    result.successful = false;
                    result.reason = "Motor safety parameters must be positive.";
                }
            }
    ...
    std::chrono::steady_clock::time_point last_motor_command_time{};
    bool motor_command_received = false;
    bool watchdog_stop_active = false;
    rclcpp::Clock clock_;
    ...
    double motor_command_timeout_sec = 0.30;
    double max_command_speed = 0.60;
    double max_command_steering = 0.58;
