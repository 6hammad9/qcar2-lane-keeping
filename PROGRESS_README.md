...
- **Everything hardware-facing runs natively** on the QCar's own Ubuntu 20.04 / ROS2 Humble install — motors, LiDAR, camera, EKF, Cartographer, AMCL, and the MPC controller all run directly on the car's onboard Jetson computer, _not_ inside Docker.
- **Docker (Isaac-ROS container) is used only for visualization** — specifically to run RViz2 so we can _see_ the map, live LiDAR scan, and the car's estimated pose. It is not part of the control/driving pipeline.
...
| ----------------- | ------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| What it is        | A fixed, saved floor plan of the room, built once by driving around with Cartographer | What the LiDAR sees _right now_, updated continuously |
